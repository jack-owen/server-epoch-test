...
test('return valid time', async () => {
  ...
  const { body } = await request.get('/time').set('Authorization', authToken)
    .expect(200);
...
test('return default prometheus metrics', async () => {
  const { text } = await request.get('/metrics').set('Authorization', authToken)
    .expect(200);
...
describe('Forbidden for invalid token', () => {
  ...
  test('Forbidden wrong token', async () => {
    const { body } = await request.get('/time').set('Authorization', 'sausages')
      .expect(403);
